...
# @return none
def attach_database(db_name,curs):
    global dbCount
    global listDB
...
#
# @return none
def close_connection(curs,conn):
    curs.close()
    conn.close()
...
def get_column_names(table_name,curs):
...
# @return none
def merge_table(table_name, column_names, db_name,curs,conn):
...
def merge_databases(mainDB, otherDBs):#list of databases
    print (f'mainDB {mainDB}')
    print (f'otherDBs {otherDBs}')
    ...
    listTable = get_table_names(curs)  # Get the table names
    ...
    for i in range(0, len(otherDBs)):
        ...
        curs = conn.cursor()
        temp = get_table_names(curs)  # Get the current list of tables
    ...
    for i in range(0, len(otherDBs)):
        attach_database(otherDBs[i],curs)  # Attach other databases

    # Merge databases
    for i in range(0, len(listDB)):
        for j in range(0, len(listTable)):
            columns = list_to_string(get_column_names(listTable[j],curs))  # get columns
            merge_table(listTable[j], columns, listDB[i],curs,conn)

    conn.commit()  # Commit changes
    close_connection(curs,conn)  # Close connection
    print("Databases finished merging. Time elapsed: %.3f" % (time.time() - startTime))
